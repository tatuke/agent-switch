...
### 阶段 2：两种模式核心（第 3-4 周）
- [ ] 模式一：Agent 自打包 bundle 读取与注入（`astp bundle`）
- [ ] 模式二：`astp onboard` 交互式配置向导
  - 记忆范围选择（手动/自动/跳过）
  - 外部记忆组件接入（Obsidian、Notion 等）
  - 全量组件默认复制（identity/tools/principles/cron）
- [ ] 注入逻辑完善（解析 bundle + onboard 产物到目标平台）
